...
def _parse_epsg(s: str) -> str:
    if not s:
        return ""
    t = str(s).strip()
    # Handle ADV URNs used in DE, e.g. "urn:adv:crs:EPSG::25832*DE_DHN92_NH"
    # PROJ/pyproj can't parse the "*DE_DHN92_NH" suffix, but the horizontal EPSG is still valid.
    if t.lower().startswith("urn:adv:crs:epsg::"):
        t = t.split("*", 1)[0]
...
def normalize_target_crs(tgt_crs_raw: str, export_3d: bool) -> str:
    ...
    # Ohne vertikales Datum: wenn wir ein EPSG extrahieren konnten, gib ein pyproj-taugliches EPSG zurück
    if hz.isdigit():
        return f"EPSG:{hz}"
...
def normalize_source_crs(src_crs_raw: str) -> str:
    ...
    if not s:
        return s
    hz = _parse_epsg(s)
    # If we extracted a clean EPSG code, feed pyproj with "EPSG:<code>" instead of an unknown URN.
    # This makes exports robust even when World["CRS"] stores an ADV URN with vertical suffix.
    if hz.isdigit():
        s = f"EPSG:{hz}"
    z = _world_z_epsg()
