...
def _resolve_image_path_primary(gml_path: str, uri: str) -> str:
    """
    Resolve texture image URIs to local filesystem paths.

    CityDB exports commonly place textures next to the GML in an "appearance"
    folder (lowercase). Some producers use "Appearance" (capitalized).
    We try both.
    """
    if not uri:
        return ""
    ...
    candidates = [
        base / uri,  # relativ zur GML (inkl. "appearance/..." wenn so referenziert)
        base / "appearance" / Path(uri).name,
        base / "Appearance" / Path(uri).name,
        base / "appearance" / uri,
        base / "Appearance" / uri,
    ]
...
def _resolve_image_path(gml_path: str, uri: str) -> str:
    # Keep compatibility: some older code paths imported this duplicate symbol.
    # Delegate to the primary resolver defined at the top of this module.
    return _resolve_image_path_primary(gml_path, uri)
...
def parse_parameterized_textures(root, gml_path: str):
    ...
    def parse_texcoordlist(tcl, targets, img_resolved, ptex_by_ring, ptex_by_poly, poly_to_image, uv_start_by_ring):
        """
        ...
        while i < len(kids):
            ...
            if rid:
                ...
                if rid and uvs and rid not in uv_start_by_ring:
                    uv_start_by_ring[rid] = (float(uvs[0][0]), float(uvs[0][1]))
        ...
        for assoc in ptx.iter():
            ...
            for tcl in assoc.iter():
                ...
                parse_texcoordlist(tcl, assoc_targets, img_resolved,
                                   ptex_by_ring, ptex_by_poly, poly_to_image, uv_start_by_ring)
    ...
    return ptex_by_ring, poly_to_image, ptex_by_poly, uv_start_by_ring
